...
if (strstr($_SERVER['HTTP_HOST'], 'lndo.site')) {
	...
	define('MILD_MEGAMENU_VERSION', '1.1.72');
}
			...
			<?php if ($count > 0): ?>
				...
				<ul>
					<?php foreach ($posts as $id): ?>
						<li><a
								href="<?php echo esc_url(get_edit_post_link($id)); ?>"><?php echo esc_html(get_the_title($id)); ?></a>
							(ID: <?php echo esc_html($id); ?>)</li>
					<?php endforeach; ?>
				</ul>
				<form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
					...
					<p><input type="submit" class="button button-primary"
							value="Convert all <?php echo esc_attr($count); ?> post(s)"></p>
				</form>
			<?php else: ?>
